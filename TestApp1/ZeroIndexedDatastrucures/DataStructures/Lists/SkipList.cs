...
namespace DataStructures.Lists
{
    ...
    public class SkipList<T> : ICollection<T>, IEnumerable<T> where T : IComparable<T>
    {
        ...
        public bool Remove(T item, out T deleted)
        {
            ...
            if (current.Value.IsEqualTo(item) == false || current == _firstNode)
            {
                deleted = default(T);
                return false;
            }
            ...
            // Decrement level
            while (_currentMaxLevel > 1 && _firstNode.Forwards[_currentMaxLevel] == _firstNode)
                --_currentMaxLevel;
        ...
        public bool Find(T item, out T result)
        {
            ...
            if (current.Value.IsEqualTo(item) && current != _firstNode)
            {
